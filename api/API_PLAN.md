...
{
   "scheduleQuotaUsed": 3,
   "scheduleQuotaLimit": 15
}
```

- `scheduleQuotaUsed`: 用户当前的课程表总数（含软删除）。
- `scheduleQuotaLimit`: 配额上限，`null` 表示无限制（`sync_unlimited = true` 的用户）。

### 服务端配置

| 配置项                  | 默认值  | 描述                   |
|:---------------------|:-----|:---------------------|
| `sync.scheduleQuota` | `15` | 每用户允许的最大课程表总数（含软删除）。 |

管理员可在数据库中将用户的 `sync_unlimited` 设为 `true` 以解除限制。

---

## 7. 云同步逻辑

客户端应使用现有的 CRUD API 实现同步功能。
...
4. **上传本地更改**:
   ...
   > **配额限制**: 当用户的课程表总数（含软删除）达到上限时，`PUT /api/v1/schedules/{id}` 对**新建**请求返回
   `403 Forbidden`（已存在的课程表不受影响，仍可正常更新）。客户端应跳过（而非中止）被拒绝的上传，并向用户展示提示。软删除某课程表后再新建不能绕过此限制，因为配额基于全部历史行数计算。
