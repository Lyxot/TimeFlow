...
fun EditCourseScreen(
    ...
    val validWeeks = (1..schedule.totalWeeks()).toMutableList().let {
        it - schedule.courses.filter {
            it != initValue && it.time.start <= course.value.time.end && it.time.end >= course.value.time.start
        ...
        mutableStateOf(
            if (course.value.time.start > course.value.time.end) {
                ...
                schedule.courses.none {
                    it != initValue && it.time.start <= course.value.time.end && it.time.end >= course.value.time.start && it.week.week.any { it in course.value.week.week }
                }
            }
        )
    }
...
fun EditCourseContent(
    ...
    LaunchedEffect(course) {
        ...
        isTimeValid.value = if (course.time.start > course.time.end) {
            ...
            schedule.courses.none {
                it != initValue && it.time.start <= course.time.end && it.time.end >= course.time.start && it.week.week.any { it in course.week.week }
            }
        }
        isWeekValid.value =
                ...
                time = range, week = WeekList(
                    week = course.week.week.filter {
                        it in (1..schedule.totalWeeks()).toMutableList().let {
                            it - schedule.courses.filter {
                                it != initValue && it.time.start <= range.end && it.time.end >= range.start
                            }.flatMap { it.week.week }
                        }
                    }
                )
